...
しばらくするとディスプレイに「Ysynth4」の文字が上から降りてくる起動画面が表示されます。USBメモリに/sf2/,/midi/,/timidity_cfg/フォルダが存在しない場合は自動的に生成します。また、もし/sf2/フォルダに"FluidR3_GM.sf2"が存在しない場合自動的にコピーします。

USBメモリが認識できていない場合は文字の代わりに四角が降ってきます。
...
それぞれの項目で表示されるデータはUSBメモリ内の/sf2/フォルダと/midi/フォルダを参照しています。フォルダが存在しない場合は自動的に生成します。また、もし/sf2/フォルダに"FluidR3_GM.sf2"が存在しない場合自動的にコピーします。Ysynth4のメモリは1GBなのでサウンドフォントは1つあたり500MB程度にしてください。対応していないサウンドフォントの場合は選択できないか、選択しても音が出ません。
...
## 履歴
    ...
    [2019/08/24] - 調整(Ver.1.2)
    [2019/08/25] - 調整(Ver.1.3)、README.mdを編集
    [2019/09/06] - 調整(Ver.1.6)    
    [2019/09/07] - WiFi設定機能を追加(Ver.1.75)、README.mdを編集
    [2019/09/09] - WiFi設定機能を修正(Ver.1.8)、README.mdを編集
    [2019/09/11] - ガーバーデータを追加(Rev.2.2)
    [2019/09/13] - 描画のマルチスレッド化(Ver.1.9)
    [2019/09/14] - 描画のマルチスレッド化調整及びディスプレイフリーズ対策追加(Ver.1.91)
    [2019/10/14] - セットアップ時に自前でTimidity++version2.15.0をビルドするように(Ver1.94)